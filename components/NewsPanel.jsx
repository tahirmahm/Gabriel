...
function highlightKeywords(text) {
  ...
  for (const kw of HIGH_THREAT_KEYWORDS) {
    result = result.replace(new RegExp(`(${kw})`, 'gi'), '|||HIGH:$1|||');
  }
  for (const kw of MEDIUM_THREAT_KEYWORDS) {
    result = result.replace(new RegExp(`(${kw})`, 'gi'), '|||MED:$1|||');
  }
...
export default function NewsPanel({ onNewsClassified }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', background: '#070709' }}>
      {/* Header */}
      <div style={{
        padding: '6px 8px', borderBottom: '1px solid #002200',
        fontFamily: '"Share Tech Mono", monospace', fontSize: 10,
        color: '#00ff41', letterSpacing: 2, flexShrink: 0, background: '#030303',
        display: 'flex', justifyContent: 'space-between', alignItems: 'center',
      }}>
        <span>◈ RSS INTEL FEED</span>
        <span style={{ fontSize: 8, color: '#004400' }}>{newsItems.length} ITEMS</span>
      </div>

      {/* Filter bar */}
      <div style={{
        display: 'flex', gap: 3, padding: '4px 6px', overflowX: 'auto',
        borderBottom: '1px solid #001a00', flexShrink: 0, scrollbarWidth: 'none',
      }}>
        {['all', 'threats', 'critical', ...RSS_SOURCES].map((f) => (
          <button key={f} onClick={() => setFilter(f)} style={{
            padding: '2px 6px', whiteSpace: 'nowrap',
            background: filter === f ? '#003300' : 'transparent',
            border: `1px solid ${filter === f ? '#00ff41' : '#002200'}`,
            color: filter === f ? '#00ff41' : '#004400',
      ...
      {/* Ticker */}
      <div style={{
        borderTop: '1px solid #002200', padding: '3px 6px',
        background: '#030303', overflow: 'hidden', flexShrink: 0,
      }}>
        <TickerBar items={newsItems.filter((i) => i.classification.level >= 2)} />
      </div>
    </div>
  );
}
...
function NewsItem({ item }) {
  ...
  return (
    <div
      ...
      style={{
        padding: '5px 8px', borderBottom: '1px solid #001a00',
        cursor: 'pointer', background: levelBg,
        borderLeft: `2px solid ${levelBorder}`,
      }}
    >
      <div style={{ display: 'flex', gap: 6, alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 4, marginBottom: 2 }}>
            <span style={{ fontSize: 9, color: srcColor, textTransform: 'uppercase', flexShrink: 0, fontFamily: '"Share Tech Mono", monospace' }}>
              [{item.source}]
            </span>
          ...
          <div style={{ fontSize: 11, fontFamily: '"Share Tech Mono", monospace', color: '#00cc33', lineHeight: 1.3, wordBreak: 'break-word' }}>
            {highlightKeywords(item.title)}
          </div>
          {expanded && item.description && (
            <div style={{ fontSize: 10, color: '#006622', marginTop: 4, lineHeight: 1.4, fontFamily: '"Share Tech Mono", monospace', borderTop: '1px solid #001a00', paddingTop: 4 }}>
          ...
          {item.classification.tags.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, marginTop: 3 }}>
              {item.classification.tags.slice(0, 3).map((tag) => (
                <span key={tag} style={{ fontSize: 8, padding: '1px 4px', border: '1px solid #ff440066', color: '#ff4400', fontFamily: '"Share Tech Mono", monospace' }}>
                  {tag}
                </span>
              ))}
            </div>
...
function TickerBar({ items }) {
  if (!items.length) return (
    <div style={{ fontSize: 9, color: '#002200', fontFamily: '"Share Tech Mono", monospace' }}>◌ MONITORING FEEDS...</div>
  );
  const text = items.map((i) => `⚠ ${i.source.toUpperCase()}: ${i.title}`).join('   ║   ');
  return (
    <div style={{ overflow: 'hidden', whiteSpace: 'nowrap' }}>
      <span style={{ display: 'inline-block', animation: 'ticker 60s linear infinite', fontSize: 9, color: '#ff9500', fontFamily: '"Share Tech Mono", monospace' }}>
        {text}
      </span>
    </div>
  );
